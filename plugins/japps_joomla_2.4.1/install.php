...
/**
	...
	private function _postProcessing() {
			...
			try {
				...
				$this->_joobiUserMessage( "Looks like Joobi encountered some errors while installation! <br> Please Contact Joobi Support with the following <br>".$e->getMessage());
				$this->_joobiErrorLog("_postProcessing() : ".$e->getMessage());
				$this->_joobiInfoLog("_postProcessing() : --- Exception Trace---");
				$this->_joobiInfoLog("_postProcessing() : ".$e->getTraceAsString());
				return false ;
			}
...
/**
	...
	private function _usefullInitialInfo() {
		...
		private function _scanFolder()
		{
			...
			catch (Exception $e) { throw new outsideCallException( "2. _scanFolder() : Something went wrong during moving code, here is the error message : " . $e->getMessage());  }//trycatch block ends
...
// 				}
				else if($this->cmsName == 'joomla')
				{
					$this->_joobiInfoLog("using joomla function for extracting joobi folder");
					// extract and move at the same time to root
					if( ! JArchive::extract( $this->_joobiTarGzSourcePath ,  $destinationPath ) ) throw new fileException("moving code from ".$destinationPath." to ".$this->_joobiTarGzSourcePath." failed");
				}// if else if
//				else if($this->cmsName == 'wordpress')
//				{
...
//				}
			...
			catch (Exception $e) { throw new Exception( "2. _extractAndMoveJoobi() : Problem while extracting " . $e->getMessage() );  }
...
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _nodeInstallFunctions 1 _runCustomInstall :" , FILE_APPEND );
		
		try{
			$this->_joobiInfoLog("_nodeInstallFunctions()");

			$installClass = JOOBI_DS_ROOT . $this->joobi . DS .'node' . DS . 'install' . DS . 'class' . DS . 'install.php';
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _nodeInstallFunctions 2 _runCustomInstall :" . $installClass , FILE_APPEND );
				
			
			if ( $this->_checkFile( $installClass, false ) ) {
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _nodeInstallFunctions 3 _runCustomInstall :" . $installClass , FILE_APPEND );
...
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _nodeInstallFunctions 4 _runCustomInstall :" . $installClass , FILE_APPEND );				
...
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _runCustomInstall :" . $nodeName , FILE_APPEND );

		$this->_joobiInfoLog( "1. _runCustomInstall()" );
		$nameOfNode = "";
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling _runCustomInstall :" . print_r( $this->_nodesListPackageTXTA, true ) , FILE_APPEND );
...
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling install function for node :" . $nodeName , FILE_APPEND );
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "calling class name :" . $completeName , FILE_APPEND );
						$this->_joobiInfoLog( "calling install function for node :" . $nodeName . " with newInstall flag :" . $customObj->newInstall );
						
						if ( empty($nodeInstallC) ) continue;
						
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "after check empty :" , FILE_APPEND );
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". print_r( get_class( $nodeInstallC ) , true ) , FILE_APPEND );
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". print_r( get_class_methods( $nodeInstallC ) , true ) , FILE_APPEND );
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". print_r( get_class_vars( get_class( $nodeInstallC ) ) , true ) , FILE_APPEND );
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". print_r( $nodeInstallC , true ) , FILE_APPEND );

						if ( method_exists( $nodeInstallC, 'install' ) ) {

// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". "after method_exists :" , FILE_APPEND );

							$nodeInstallC->install( $customObj );
							
						}//endif
						
// file_put_contents( JOOBI_DS_ROOT . 'joobi-installation-custom-fct.log', "\n". " install function success " , FILE_APPEND );

						$this->_joobiInfoLog( " install function success " );

						if( method_exists( $nodeInstallC, 'addExtensions') ) $nodeInstallC->addExtensions();

						$this->_joobiInfoLog( " addExtensions function success " );
...
/**
	...
	private function _checkFile( $filePath="", $throwErr=true ) {
		...
		elseif ( ! file_exists( $filePath ) ) {
			if( $throwErr ) throw new fileException("file does not exist at ".$filePath); else return false;
		} elseif ( ! is_readable($filePath) ) {
			if ( $throwErr ) throw new fileException("file is not readable at ".$filePath); else return false;
		}//endif
